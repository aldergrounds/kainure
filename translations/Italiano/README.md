...
- **`main_file`**: Questo è il punto di ingresso della tua gamemode. Kainure inizierà a eseguire il codice a partire da questo file. Per impostazione predefinita, può essere `.js` o `.ts` (se TypeScript è attivato). Il percorso `./` rappresenta la root della tua gamemode.

- **`encoding.enabled`**: Definisce se Kainure deve convertire le stringhe tra il formato UTF-8 (standard di JavaScript) e la codifica del tuo server. Se impostato su `false`, il framework utilizzerà lo standard `65001` (UTF-8).

- **`encoding.target`**: Definisce la codifica target.
   - **Windows**: È obbligatorio l'uso del **numero della Codepage** (es: `1252`).
   - **Linux**: È obbligatorio l'uso del **nome della Codepage** (es: `WINDOWS-1252`).

- **`typescript.enabled`**: Cambia in `true` se desideri scrivere il tuo codice in TypeScript. Kainure transpilerà automaticamente i file `.ts` in `.js`.
- **`typescript.output_dir`**: Definisce la directory in cui verranno salvati i file JavaScript compilati.
...
Dichiara una callback (public) di SA-MP, permettendo al tuo codice di reagire agli eventi di gioco.

**Firme dei Parametri:** Affinché Kainure converta i tipi correttamente, è obbligatorio fornire una firma come secondo parametro nel caso in cui la callback possieda argomenti.
- **`i`**: Intero
- **`f`**: Float
- **`s`**: String
- **`b`**: Booleano

**Validazioni ed Errori:** Kainure impedisce l'esecuzione se:
1. La funzione possiede parametri, ma non viene fornita alcuna firma.
2. La quantità di caratteri nella firma è diversa dalla quantità di parametri definiti nella funzione.
3. La firma contiene tipi non validi (diversi da `i, f, s, b`).

**Valori di Ritorno:**
- `return 1;` o `return true;`: Permette alla callback di continuare a essere eseguita in altri script (se presenti). Questo è il comportamento predefinito se non viene restituito nulla.
- `return 0;` o `return false;`: Impedisce l'esecuzione della callback in altri script.

```javascript
// Public semplice senza parametri
...
Questa funzione viene utilizzata per registrare comandi che **non sono stati creati nell'ambiente Kainure**, ma che esistono in altri script.

Registrando un comando come esterno, Kainure garantisce che le callback globali dei comandi (`OnPlayerCommandReceived` e `OnPlayerCommandPerformed`) vengano attivate correttamente per questi comandi, integrandoli nel flusso del framework.

```javascript
// Registrazione dei comandi esistenti in un filterscript
