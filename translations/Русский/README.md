...
- [**Kainure**](#kainure)
  ...
  - [Как установить и использовать](#как-установить-и-использовать)
    ...
    - [3. Настройка (config.json)](#3-настройка-configjson)
...
Внутри папки `Kainure/` вы найдете файл `config.json`. Если он не существует, плагин создаст его автоматически при первом запуске. Этот файл управляет поведением фреймворка.
...
- **`main_file`**: Это точка входа вашего гейммода. Kainure начнет выполнение кода с этого файла. По умолчанию это может быть `.js` или `.ts` (если включен TypeScript). Путь `./` представляет корень вашего гейммода.

- **`typescript.enabled`**: Измените на `true`, если вы хотите писать свой код на TypeScript. Kainure автоматически транспилирует файлы `.ts` в `.js`.

- **`typescript.output_dir`**: Определяет директорию, где будут сохранены транспилированные файлы JavaScript.

- **`typescript.auto_install`**: Если `true`, Kainure проверит, присутствует ли компилятор TypeScript (`typescript`) в `node_modules` вашего проекта. Если нет, он установит его автоматически, упрощая начальную настройку.
...
#### `Public`
Объявляет callback (public) SA-MP, позволяя вашему коду реагировать на события игры.

**Сигнатуры параметров:**
**Крайне важно** предоставить сигнатуру для параметров, которые являются `string` или `float`, чтобы Kainure мог правильно конвертировать типы.
- **`string`**: `(param = "s")`
- **`float`**: `(param = "f")`
- `int` и `bool` не требуют сигнатуры.

**Возвращаемые значения:**
- `return 1;` или `return true;`: Позволяет callback-у продолжить выполнение в других скриптах (если есть). Это поведение по умолчанию, если ничего не возвращено.
- `return 0;` или `return false;`: Предотвращает выполнение callback-а в других скриптах.

```javascript
// Простой Public
Public('OnPlayerSpawn', (playerid) => {
    Native.SendClientMessage(playerid, -1, "Вы заспавнились на сервере.");
    return true;
});

// Public с сигнатурой string и float
Public('PublicCustom', (text = "s", value = "f") => {
    console.log(`Текст: ${text}, Значение: ${value}`);
});
```
